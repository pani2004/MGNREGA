...
class DataController {
  ...
  async getCurrentData(req, res) {
    try {
      ...
      const formattedData = this.formatDataForResponse(latestData);
  ...
  async getHistoricalData(req, res) {
    try {
      ...
      const formattedData = historicalData.map(data => this.formatDataForResponse(data));
    ...
    try {
      ...
      const comparativeData = await Promise.all(
        idsArray.map(async (districtId) => {
          ...
          return latestData ? this.formatDataForResponse(latestData) : null;
        })
      );
   ...
   */
  async getStateAggregatedData(req, res) {
    try {
